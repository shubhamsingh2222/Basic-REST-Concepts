...
app.get('/random/', async(req,res)=>
{
  ...
  // res.status(200).json({
  //   id: randomJokes.id,
  //   jokeText: randomJokes.jokeText,
  //   jokeType: randomJokes.jokeType
  // });
...
app.get('/jokes/:id', async(req,res)=>
{
  ...
  function findById(jokeId)
  {
    return jokes.find((joke) => joke.id == jokeId)
  }
  let data = await findById(jokeId)
...
app.put('/jokes/:id', async(req, res) =>
{
  ...
  const searchIndex = jokes.findIndex((i) => i.id == indexId);
...
{
  const indexId = req.params.id;
  const existingJoke = jokes.find((joke) => joke.id == indexId);
  const newData = {
    id: indexId,
...
app.delete('/jokes/:id', async(req,res)=>
{
  ...
  let searchIndex = jokes.findIndex((i)=> i.id == id)
  if(searchIndex > -1)
  {
    jokes.splice(searchIndex, 1);
    res.sendStatus(200);
  }else{
    res.status(404).json({error: `Jokes with ${id} not found. No jokes were deleted`});
  }
})
//8. DELETE All jokes
